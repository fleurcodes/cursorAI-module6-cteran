...
test.describe('Screen Reader Error Announcements', () => {
  ...
  test('success screen has role="status" for polite screen reader announcement', async ({
    page,
  }) => {
    await page.route('/api/register', (route) =>
      route.fulfill({
    ...
    await expect(reg.successScreen).toHaveAttribute('role', 'status');
    await expect(reg.successScreen).toHaveAttribute('aria-live', 'polite');
  });
});
